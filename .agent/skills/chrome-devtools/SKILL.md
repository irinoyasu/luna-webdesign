...
cd .agent/skills/chrome-devtools/scripts
...
Without ImageMagick, screenshots >5MB will not be compressed (may fail to load in Gemini/Antigravity).
...
All scripts are in `.agent/skills/chrome-devtools/scripts/`
...
Screenshots are **automatically compressed** if they exceed 5MB to ensure compatibility with Gemini API and Antigravity (which have 5MB limits). This uses ImageMagick internally:
...
2. Verify in `.agent/skills/chrome-devtools/scripts/` directory
